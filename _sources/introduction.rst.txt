...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/debussy_images.git
...
* `Claude Debussy – Images <https://github.com/DCMLab/debussy_images/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by five files with identical names, each in its own folder. For example, the first piece of the first set, *Lent, mélancolique et doux*, has the following files:

-  ``MS3/l087-01_images_lent.mscx``: Uncompressed MuseScore 3.6.2 file including the music and annotation labels.
-  ``notes/l087-01_images_lent.notes.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/l087-01_images_lent.measures.tsv``: A table with relevant information about the measures in the score.
-  ``chords/l087-01_images_lent.chords.tsv``: A table containing layer-wise unique onset positions with the musical markup (such as dynamics, articulation, lyrics, figured bass, etc.).
-  ``harmonies/l087-01_images_lent.harmonies.tsv``: A table of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/l087-01_images_lent.tsv')
   notes = ms3.load_tsv('notes/l087-01_images_lent.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/debussy_images/issues>`__ and feel free to fork and submit pull requests.
